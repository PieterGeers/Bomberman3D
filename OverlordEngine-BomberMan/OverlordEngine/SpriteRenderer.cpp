...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		buffDesc.ByteWidth = m_BufferSize * sizeof(SpriteVertex);
		buffDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		buffDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		buffDesc.MiscFlags = 0;
		
		const auto hr = gameContext.pDevice->CreateBuffer(&buffDesc, nullptr, &m_pVertexBuffer);
		if (Logger::LogHResult(hr, L"SpriteRenderer::Draw > Vertexbuffer creation failed!"))
			return;
	}
	//------------------------
	//Sort Sprites
	//SORT BY TEXTURE
	sort(m_Sprites.begin(), m_Sprites.end(), SpriteSortByTexture);
	...
	memcpy(mappedSubResource.pData, m_Sprites.data(), m_Sprites.size() * sizeof(SpriteVertex));
