...
void TerrainComponent::CalculateVerticesAndIndices()
{
	...
	m_VecVertices.resize(m_NrOfRows*m_NrOfColumns);
	...
	int nrQuadsColums = m_NrOfColumns - 1;
	...
	for (unsigned row = 0; row < m_NrOfRows; ++row)
	{
		...
		for (unsigned col = 0; col < m_NrOfColumns; ++col)
		{
			...
			m_VecVertices[vertexId].TexCoord.x = static_cast<float>(col) / static_cast<float>(m_NrOfColumns - 1);
			m_VecVertices[vertexId].TexCoord.y = static_cast<float>(row) / static_cast<float>(m_NrOfRows - 1);
	...
	{
		for (int col = 0; col < nrQuadsColums; ++col)
		{
	...
	//For each face...
	for (unsigned i = 0; i < m_VecIndices.size(); i += 6)
	{
	...
	for (unsigned row = 0; row < m_NrOfRows; ++row)
	{
		for (unsigned col = 0; col < m_NrOfColumns; ++col)
		{
			...
			XMFLOAT3 normalFloat3;
			XMStoreFloat3(&normalFloat3, sum);
			int vertexId = row * m_NrOfColumns + col;
			m_VecVertices[vertexId].Normal = normalFloat3;
		}
	}
}
...
void TerrainComponent::CreatePxHeightField()
{
	...
	samples.resize(m_NrOfVertices);
	for (unsigned i = 0; i < m_NrOfVertices; ++i)
		samples[i].height = static_cast<physx::PxI16>(m_VecVertices[i].Position.y);
	...
	auto aHeightField = physX->createHeightField(hfDesc);
	auto hfGeom = physx::PxHeightFieldGeometry(aHeightField, physx::PxMeshGeometryFlags(), 1.0f, m_Width / m_NrOfColumns, m_Depth / m_NrOfRows);
	
	auto bouncyMaterial = physX->createMaterial(.5f, .5f, .5f);
	auto m_pActor = physX->createRigidStatic(physx::PxTransform(-m_Width / 2.0f, 0, m_Depth / 2.0f, physx::PxQuat(physx::PxPiDivTwo, physx::PxVec3(0, 1, 0))));
	auto pShape = m_pActor->createShape(hfGeom, *bouncyMaterial);
	m_pActor->userData = this;
	pShape->setSimulationFilterData(physx::PxFilterData(CollisionGroupFlag::Group0, 0, 0, 0));
	GetGameObject()->GetScene()->GetPhysxProxy()->GetPhysxScene()->addActor(*m_pActor);
}
