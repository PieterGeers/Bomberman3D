...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float3 normal, float2 texCoord)
{
	...
	//Step 4. Assign texCoord to (GS_DATA object).TexCoord/70
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float3 basePoint, top, left, right, spikeNormal;//80

	//Step 1. Calculate The basePoint
    float3 m0 = 0.5f * (vertices[0].Position + vertices[1].Position);
    float3 m1 = 0.5f * (vertices[1].Position + vertices[2].Position);
    float3 m2 = 0.5f * (vertices[2].Position + vertices[0].Position);
   
    basePoint = (m0 + m1 + m2) / 3;

	//Step 2. Calculate The normal of the basePoint
    float3 e0 = vertices[2].Position - m0;
    float3 e1 = vertices[1].Position - m2;
    float3 e2 = vertices[0].Position - m1;
    

	float3 n1 = normalize(cross(e0, e1));
    float3 n2 = normalize(cross(e1, e2));
    float3 n3 = normalize(cross(e2, e0));

    float3 normal = (n1 + n2 + n3) / 3;
	//100
	//Step 3. Calculate The Spike's Top Position
    top = basePoint + (normal * 8);
    ...
    float3 t0 = top - left;
    float3 t1 = top - right;
    spikeNormal = normalize(cross(t0, t1));
