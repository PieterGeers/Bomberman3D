...
VS_OUTPUT VS(VS_INPUT input){
    ...
    for (int i = 0; i < 4; ++i)
    {
        int currentIdx = input.BlendIndices[i];
        if (currentIdx > -1)
        {
            transformedPosition += mul(originalPosition, gBones[currentIdx]) * input.BlendWeights[i];
            transformedNormal += input.BlendWeights[i] * input.normal;
        }
    }
