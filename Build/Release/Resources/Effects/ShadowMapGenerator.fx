...
float4 ShadowMapVS_Skinned(float3 position:POSITION, float4 BoneIndices : BLENDINDICES, float4 BoneWeights : BLENDWEIGHTS) : SV_POSITION
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        int currentIdx = BoneIndices[i];
        if (currentIdx > -1)
        {
            transformedPosition += mul(float4(position, 1.0f), gBones[currentIdx]) * BoneWeights[i];
        }
    }
    transformedPosition.w = 1;

    float4 output = mul(transformedPosition, mul(gWorld, gLightViewProj));
